...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 帖子管理 <span
        class="c-gray en">&gt;</span> 即时信息回帖 <a id="btn-refresh" class="btn btn-success radius r"
...
<div class="page-container">
    <form id="form-search" class="text-c"> 日期范围：
        ...
        <input type="text" class="input-text" style="width:250px" placeholder="输入帖子ID、用户姓名等信息" id="searchKey"
               name="searchKey">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">共有数据：<strong id="instantNewsReplyListCount">0</strong> 条</span>
    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        $('.table').dataTable({
            ...
            "columns": [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                            if( row.status == 0 ){
                                return "<a title=\"屏蔽\" href=\"javascript:;\" onclick=\"instantNewsReply_stop(this," + row.id + ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe631;</i></a>" +
                                    "<a title=\"删除\" href=\"javascript:;\" onclick=\"instantNewsReply_del(this,"+row.id+ ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe6e2;</i></a>";
                            }else if( row.status ==1 ){
                                return "<a title=\"正常\" href=\"javascript:;\" onclick=\"instantNewsReply_start(this," + row.id + ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe604;</i></a>" +
                                    "<a title=\"删除\" href=\"javascript:;\" onclick=\"instantNewsReply_del(this,"+row.id+ ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe6e2;</i></a>";
                            }
                    }
                }
        ...
        instantNewsReply_count();
    })



    /*统计用户反馈数*/
    function instantNewsReply_count() {
        $.ajax({
            url: "/instantNewsReply/count",
            type: "GET",
            success: function (data) {
                ...
                $("#instantNewsReplyListCount").html(data.recordsTotal);
            },
    ...
    /*即时帖子-屏蔽*/
    function instantNewsReply_stop(obj, id) {
        layer.confirm('确认要屏蔽吗？',{icon:0},function(index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/instantNewsReply/stop/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
    ...
    /*即时帖子-正常*/
    function instantNewsReply_start(obj, id) {
        layer.confirm('确认要正常吗？',{icon:0},function(index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/instantNewsReply/start/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
                    if (data.success != true) {
    ...
    /*帖子-删除*/
    function instantNewsReply_del(obj, id) {
        layer.confirm('确认要删除id为\'' + id + '\'的即时帖子吗？', {icon: 0}, function (index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/instantNewsReply/remove/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
                    if (data.success != true) {
    ...
    function datadel() {
        ...
        layer.confirm('确认要删除所选的' + count + '条数据吗？', {icon: 0}, function (index) {
            ...
            $.ajax({
                type: 'PUT',
                url: '/instantNewsReply/remove/' + ids,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
                    if (data.success != true) {
