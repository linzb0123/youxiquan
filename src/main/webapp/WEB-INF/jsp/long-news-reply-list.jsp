...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 帖子管理 <span
        class="c-gray en">&gt;</span> 长期信息回帖 <a id="btn-refresh" class="btn btn-success radius r"
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">共有数据：<strong id="longNewsReplyListCount">0</strong> 条</span>
    </div>
...
<script type="text/javascript">
        ...
        $('.table').dataTable({
            ...
            "columns": [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                            if( row.status == 0 ){
                                return "<a title=\"屏蔽\" href=\"javascript:;\" onclick=\"longNewsReply_stop(this," + row.id + ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe631;</i></a>" +
                                    "<a title=\"删除\" href=\"javascript:;\" onclick=\"longNewsReply_del(this,"+row.id+ ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe6e2;</i></a>";
                            }else if( row.status ==1 ){
                                return "<a title=\"正常\" href=\"javascript:;\" onclick=\"longNewsReply_start(this," + row.id + ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe604;</i></a>" +
                                    "<a title=\"删除\" href=\"javascript:;\" onclick=\"longNewsReply_del(this,"+row.id+ ")\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\" style='font-size:16px;'>&#xe6e2;</i></a>";
                            }
                    }
                }
    ...
    /*统计用户反馈数*/
    function longNewsReply_count() {
        $.ajax({
            url: "/longNewsReply/count",
            type: "GET",
            success: function (data) {
                ...
                $("#longNewsReplyListCount").html(data.recordsTotal);
            },
            error: function (XMLHttpRequest) {
                layer.alert('数据处理失败! 错误码:' + XMLHttpRequest.status, {title: '错误信息', icon: 2});
            }
    ...
    /*即时帖子-屏蔽*/
    function longNewsReply_stop(obj, id) {
        layer.confirm('确认要屏蔽吗？',{icon:0},function(index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/longNewsReply/stop/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
    ...
    /*即时帖子-正常*/
    function longNewsReply_start(obj, id) {
        layer.confirm('确认要正常吗？',{icon:0},function(index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/longNewsReply/start/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
                    if (data.success != true) {
    ...
    /*帖子-删除*/
    function longNewsReply_del(obj, id) {
        layer.confirm('确认要删除id为\'' + id + '\'的即时帖子吗？', {icon: 0}, function (index) {
            var index = layer.load(3);
            $.ajax({
                type: 'PUT',
                url: '/longNewsReply/remove/' + id,
                dataType: 'json',
                success: function (data) {
                    layer.close(index);
                    if (data.success != true) {
